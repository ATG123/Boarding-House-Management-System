...
$getroomnum=substr($comp,0,$actuallen);
$_SESSION["room"]=$getroomnum;
echo "<br>";

$getbednum=substr($comp,-1);
$_SESSION["bed"]=$getbednum;
echo "<br>";

$querystatus="select adhar from notifyuser where adhar='$getadhar'";
$countrows=mysql_query($querystatus) or die("Error,Update Status Query Failed");
$count=0;
...
if($count>0)
{
echo "<script language='javascript'>alert('YOU CANNOT ALLOCATE TWO SEATS TO THE SAME USER.');</script>";
}
else
{
$querystatus=" update nonacrooms set status='Blocked' where roomno='$getroomnum' and bedno='$getbednum'";
mysql_query($querystatus) or die("Error,Update Status Query Failed");
echo "<br>";

$pquerystatus="update basicformfill set display='infotaken' where adhar='$getadhar'";
mysql_query($pquerystatus) or die("Error,Update Display Query Failed");
echo "<br>";

$typeofroom=$_SESSION["typeofroom"];
$subtypeofroom=$_SESSION["subtypeofroom"];

$query = "insert into notifyuser
values('$getadhar','$typeofroom','$subtypeofroom','$getroomnum','$getbednum')";

$failurequery="update nonacrooms set status='Not Booked' where roomno='$getroomnum' and bedno='$getbednum' ";
mysql_query($query) or die(mysql_query($failurequery));

echo "<script language='javascript'>alert('ROOM HAS BEEN SUCCESSFULLY BOOKED.');</script>";
