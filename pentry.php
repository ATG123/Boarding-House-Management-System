...
$query = "select contactno from basicformfill where enquiryid='$enquiry'";
echo "<h2>" . $query;
echo "<br>";
$countrows=mysql_query($query) or die("Error,Insert Query Failed");
while ($res = mysql_fetch_array($countrows)) 
{
$tempres=$res['contactno']; 
echo $tempres;
echo "<br>";
}
...
echo "<br>";
$x=strcmp($tempres,$mob);
$y=abs($x);
echo $y;
echo "<br>";
if($y)
     echo "U R UNAUTHORIZED PERSON";
else
{
     $_SESSION["contactno"]=$tempres;
     echo "<script language='javascript'>document.location='pentrynext.php';</script>";
}
?>
</table>
</div>
