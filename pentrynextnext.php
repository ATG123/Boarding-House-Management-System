...
$getroom=$_SESSION["roomnumber"];
$getbed=$_SESSION["bednumber"];
}
$x=strcmp($answer,'YES');
$y=abs($x);
if($y)
{
  ...
  $pquery="select * from basicformfill where adhar='$getadhar' ";
  $countrows=mysql_query($pquery) or die("Error ,Select Query Failed");
  while($res = mysql_fetch_array($countrows))
  {
      $getadhar=$res['adhar'];
      $getfname=$res['firstname'];
      $getlname=$res['lastname'];
      $getemailid=$res['emailid'];
      $getcontact=$res['contactno'];
      $gettyperoom=$res['typeofroom'];
      $getsubtyperoom=$res['subtypeofroom'];
  }
  $ppquery="insert into backuptable values ('$getadhar','$getfname','$getlname','$getemailid','$getcontact','$gettyperoom','$getsubtyperoom')";

  mysql_query($ppquery) or die("Error ,Insert Query Failed");

   $query = "delete from basicformfill where adhar='$getadhar' ";
  mysql_query($query) or die("Error ,Delete Query Failed");

   $nquery = "delete from notifyuser where adhar='$getadhar' ";
  mysql_query($nquery) or die("Error ,Delete Query Failed");
 
 if($getroom>=1 && $getroom<=10)
  {
       $query = "update acrooms set status='Not Booked' where roomno='$getroom' and bedno='$getbed'";
       mysql_query($query) or die("Error ,Update Query Failed");
  }
 else
  if($getroom>=11 && $getroom<=20)
  {
       $query = "update nonacrooms set status='Not Booked' where roomno='$getroom' and bedno='$getbed'";
       mysql_query($query) or die("Error ,Update Query Failed");
  }
echo "<script language='javascript'>document.location='viewinside.html';</script>";
}
else
{
if($getroom>=1 && $getroom<=10)
  {
       $query = "update acrooms set status='Booked' where roomno='$getroom' and bedno='$getbed'";
       mysql_query($query) or die("Error ,Update Query Failed");
  }
 else
  if($getroom>=11 && $getroom<=20)
  {
       $query = "update nonacrooms set status='Booked' where roomno='$getroom' and bedno='$getbed'";
       mysql_query($query) or die("Error ,Update Query Failed");
  }

$getadhar=$_SESSION["adhar"];

$squery = "update basicformfill set display='student' where adhar='$getadhar'";
   mysql_query($squery) or die("Error ,Update Query Failed");
echo "<br>";
echo $squery;
echo "<br>";
echo "closing connections";
mysql_close($connection);
echo "<script language='javascript'>document.location='finalform.php';</script>";
}
?>
